...
export default function Index({
    ...
    // Syarat data — dikelola secara lokal agar responsif instan tanpa reload halaman
    ...
    // Sync state ketika props berubah
    useEffect(() => {
    ...
    // Handle Bidang Change — filter layanan secara instan di sisi klien
    ...
    // Handle Layanan Change — fetch syarat via API asynchronous
    const handleLayananChange = (newLayananId) => {
        ...
        fetch(`/adminBawah/get-syarat-by-layanan/${encodeURIComponent(newLayananId)}`, {
            headers: {
    ...
    // Handle Reset — bersihkan seluruh filter
    const handleResetFilter = () => {
        setCurrentBidang(bidangId || '');
        setCurrentLayanan('');
        const defaultLayanan = (allLayanan || []).filter(
            (l) => String(l.kode_bidang) === String(bidangId)
        );
        setLayananOptions(defaultLayanan);
        setSyaratData([]);
        setSelectedLayanan(null);
        setSearchQuery('');
        setCurrentPage(1);
    ...
    // Filter Syarat berdasarkan pencarian teks
    ...
    // Export PDF URL resmi Admin Bawah
    const exportPdfUrl = hasSelectedLayanan
        ? `/adminBawah/cetakSyarat/export?bidang=${encodeURIComponent(currentBidang)}&layanan=${encodeURIComponent(currentLayanan)}`
        : '#';

    return (
        <AuthenticatedLayout user={auth?.user}>
            <Head title="Cetak Syarat Layanan - Admin BKPSDM" />
            ...
            <div className="space-y-6">
                    ...
                    <div className="flex flex-col sm:flex-row gap-4 items-end">
                        ...
                        <div className="flex-shrink-0">
                            ...
                            <button
                                ...
                                className="w-full sm:w-auto inline-flex items-center justify-center gap-1.5 px-5 py-2.5 rounded-xl bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 text-xs sm:text-sm font-semibold transition-all shadow-2xs cursor-pointer"
                            >
                ...
                {/* 3. HASIL PERSYARATAN & TABEL (Bagian 8 Standard.md) */}
                {!hasSelectedLayanan ? (
                    /* PROMPT CARD BELUM MEMILIH LAYANAN (Bagian 8 Standard.md) */
                    ...
                    /* EMPTY STATE BELUM ADA SYARAT (Bagian 8 Standard.md) */
                    <div className="rounded-3xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 py-16 px-4 text-center shadow-xs">
                        <FileText className="w-10 h-10 mx-auto text-slate-300 dark:text-slate-700 mb-3" />
                        <h4 className="text-sm font-bold text-slate-800 dark:text-slate-100">
                    ...
                    /* TABEL DATA PERSYARATAN TUNGGAL (Bagian 3 & 4 Standard.md) */
                    <div className="space-y-4">
                        ...
                        {/* SINGLE DATA TABLE CARD (Bagian 4 Poin 4 Standard.md) */}
