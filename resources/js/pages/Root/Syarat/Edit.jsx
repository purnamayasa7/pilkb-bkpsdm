...
export default function RootSyaratEdit({
    syarat = null,
    syaratEfile = [],
}) {
    // Form state initialized from existing syarat
    const [namaSyarat, setNamaSyarat] = useState(syarat?.syarat || '');
    const [metode, setMetode] = useState(syarat?.metode || 'simpeg');
    const [kodeEfile, setKodeEfile] = useState(syarat?.kode_efile || '');
    const [deskripsi, setDeskripsi] = useState(syarat?.deskripsi || '');
    ...
    // Helpers
    const namaBidang = syarat?.layanan?.bidang?.nama_bidang || 'Bidang Terkait';
    const bidangId = syarat?.layanan?.bidang?.id || '';
    const namaLayanan = syarat?.layanan?.nama_layanan || 'Layanan Terkait';
    const kodeLayanan = syarat?.kode_layanan || '';
    const selectedEfileObj = syaratEfile.find((e) => e.efile === kodeEfile);

    // Back URL preserving filter context
    ...
    const handleConfirmSubmit = () => {
        ...
        router.put(
            `/root/syarat/${syarat.id}`,
            {
                syarat: namaSyarat.trim(),
                metode: metode,
                    ...
                    <div className="flex items-start gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">
                                Perbarui konfigurasi dokumen persyaratan atau integrasi e-File SIMPEG untuk layanan BKPSDM.
                            </p>
                        </div>
                    </div>
                    ...
                    <div className="px-6 py-5 border-b border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-800/30 flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-xs text-slate-400">
                                Ubah informasi persyaratan layanan dan integrasi SIMPEG pada formulir berikut
                            </p>
                        </div>
                    </div>
                    ...
                    <form onSubmit={handlePreSubmit} className="p-6 sm:p-8 space-y-6">
                        ...
                        <div>
                            ...
                            <input
                                type="text"
                                value={namaSyarat}
                                onChange={(e) => setNamaSyarat(e.target.value)}
                                required
                                placeholder="Contoh: SK Kenaikan Pangkat Terakhir, Ijazah & Transkrip Nilai..."
