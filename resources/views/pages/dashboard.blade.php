...
<div class="container-fluid px-4 mt-4">
    ...
    <div class="row">
        ...
        @if ($isAdminOpd)
        {{-- WIDGET 4 PENGAJUAN TERAKHIR ADMIN OPD --}}
        <div class="col-lg-4 mb-4">
            <div class="card card-header-actions mb-4 h-100">
                <div class="card-header">
                    Pengajuan Terakhir
                    <a class="btn btn-sm btn-primary-soft text-primary" href="{{ route('adminOpd.tiket.indexProses') }}">
                        Lihat Semua
                        <i class="ms-1" data-feather="arrow-right"></i>
                    </a>
                </div>
                <div class="card-body py-3">
                    @if (isset($pengajuanTerakhirOpd) && $pengajuanTerakhirOpd->isNotEmpty())
                    <div class="timeline timeline-xs">
                        @foreach ($pengajuanTerakhirOpd as $tiket)
                        @php
                        $waktuTeks = \Carbon\Carbon::parse($tiket->tanggal)->translatedFormat('d M');

                        $statusNama = $tiket->tahapTerakhir->statusRel->status ?? 'Tahap Awal';
                        $statusLower = strtolower($statusNama);
                        ...
                        <div class="timeline-item">
                            <div class="timeline-item-marker">
                                <div class="timeline-item-marker-text"
                                    title="{{ \Carbon\Carbon::parse($tiket->tanggal)->translatedFormat('d F Y') }}">
                                    {{ $waktuTeks }}
                                </div>
                                <div class="timeline-item-marker-indicator {{ $indicatorClass }}"></div>
                            </div>
                            <div class="timeline-item-content" style="min-width: 0;">
                                <a class="fw-bold text-primary text-decoration-none small"
                                    href="{{ route('adminOpd.tiket.indexProses') }}">
                                    #{{ $tiket->no_tiket }}
                                </a>
                                <span class="badge {{ $badgeClass }} ms-1" style="font-size: 0.65rem;">
                                    {{ $statusNama }}
                                </span>
                                <div class="small fw-semibold text-dark text-truncate"
                                    title="{{ $tiket->layanan->nama_layanan ?? '-' }}">
                                    {{ $tiket->layanan->nama_layanan ?? '-' }}
                                </div>
                                @if ($tiket->nama || $tiket->nip)
                    ...
                    <div class="text-center py-4 text-muted">
                        ...
                        <p class="mb-0 small">Belum ada riwayat pengajuan usulan</p>
                    </div>
                    @endif
                </div>
            </div>
...
@php $defaultBulan = request('bulan') ?? now()->format('Y-m'); @endphp
@push('scripts')
<script>
    $(document).ready(function() {
        ...
        // DEFAULT BULAN
        $('#monthPicker').datepicker('setDate', '{{ $defaultBulan }}');
