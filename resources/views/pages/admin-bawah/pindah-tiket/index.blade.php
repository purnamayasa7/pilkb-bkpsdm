            ...
            <div class="position-relative">
                ...
                <div id="tableContainer" style="min-height: 200px; transition: opacity 0.25s ease-in-out;">
                    ...
                    <div class="text-center py-5 text-muted">
                        <i data-feather="shuffle" class="mb-3" style="width: 48px; height: 48px; opacity: 0.5;"></i>
                        <h5 class="fw-bold text-dark">Pencarian Tiket</h5>
                        <p class="mb-0 text-muted">Silakan masukkan Nomor Tiket atau NIP pada kolom di atas untuk mencari tiket yang akan dipindahkan.</p>
                    </div>
                    @endif
                </div>
            </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function searchTiket() {
            ...
            fetch(`/adminBawah/pindah/get-data?keyword=${encodeURIComponent(keyword)}`)
                .then(res => res.json())
                .then(data => {
                    if (!data || data.length === 0) {
                        ...
                        if (tableContainer) {
                            tableContainer.innerHTML = `
                                <div class="alert alert-warning mb-0" role="alert">
                                    <i data-feather="alert-triangle" class="me-1"></i> Data tiket dengan kata kunci <strong>"${escapeHtml(keyword)}"</strong> tidak ditemukan.
                                </div>
                            `;
                            feather.replace();
                            tableContainer.style.opacity = '1';
